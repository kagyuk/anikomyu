...
if (newsList && csvUrl && csvUrl !== 'YOUR_GOOGLE_SHEETS_CSV_URL') { // 要素があり、URLが設定されている場合のみ実行
    fetch(csvUrl)
        ...
        .then(csvText => {
            ...
            const dateIndex = headers.indexOf('date'); // 'date' 列のインデックス
            const titleIndex = headers.indexOf('title'); // 'title' 列のインデックス
            const contentIndex = headers.indexOf('content'); // 'content' 列のインデックス
